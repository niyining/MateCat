	...
	renderConcordances: function(d, in_target) {
		...
		if (d.data.matches.length) {
            $.each(d.data.matches, function(index) {
                ...
                $('.sub-editor.concordances .overflow .results', segment).append('' +
					'<ul class="graysmall' + prime + '" data-item="' + (index + 1) + '" data-id="' + this.id + '">' +
						'<li class="sugg-source">' + ((disabled) ? '' : ' ' +
							'<a id="' + segment_id + '-tm-' + this.id + '-delete" href="#" class="trash" title="delete this row"></a>') +
							'<span id="' + segment_id + '-tm-' + this.id + '-source" class="suggestion_source">' + leftTxt + '</span>' +
						'</li>' +
						'<li class="b sugg-target">' +
							'<span id="' + segment_id + '-tm-' + this.id + '-translation" class="translation">' + rightTxt + '</span>' +
						'</li>' +
						'<ul class="graysmall-details"> ' + cl_suggestion + '">' + (this.match) +
