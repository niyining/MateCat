...
$.extend(UI, {
	chooseSuggestion: function(w) {
        ...
        var isIA = $(ulDataItem + w + '] ul.graysmall-details .percent').hasClass('per-blue');
		this.copySuggestionInEditarea(this.currentSegment, $(ulDataItem + w + '] li.b .translation').html(),
			$('.editor .editarea'), $(ulDataItem + w + '] ul.graysmall-details .percent').text(), isIA);
	...
	copySuggestionInEditarea: function(segment, translation, editarea, match, isIA) {
		if (typeof (decode) == "undefined") {
			decode = false;
		}
		var percentageClass = this.getPercentuageClass(match, isIA);
		if ($.trim(translation) !== '') {
    ...
    renderContributions: function(d, segment) {
        ...
        if ( d.data.hasOwnProperty('matches') && d.data.matches.length) {
            ...
            $.each(d.data.matches, function(index) {
                ...
                if ( isArtificialIntelligence ) {
                    var text = '<div style="text-align:left;font-size: 15px;">Generated by MateCat autoediting the best match. </br>The best match ( ' + percentOriginalText + ' ) is now a ' + percentText + '.</div>';
			...
			if (editareaLength === 0) {
                ...
                var copySuggestion = function() {
                    UI.copySuggestionInEditarea(segment, translation, editarea, match, is_IA);
                };
